...
# 6.0
# 7 Emissions
CumulativeEmissions                 = np.zeros(len(timeSteps))*np.nan
CumulativeEmissions[0]              = 0
...
MarketShareBiomass                  = np.zeros(len(timeSteps))*np.nan
MarketShareOil                      = np.zeros(len(timeSteps))*np.nan
OilFactor                           = np.zeros(len(timeSteps))*np.nan
BioFactor                           = np.zeros(len(timeSteps))*np.nan

# 7 Emissions
EmisPrice                           = np.zeros(len(timeSteps))*np.nan
OilEmisCost                         = np.zeros(len(timeSteps))*np.nan
VolumeOil                           = np.zeros(len(timeSteps))*np.nan
...
# 7 Emissions
Emissions                           = np.zeros(len(timeSteps))*np.nan
...
for t in range(len(timeSteps)):
    ...
    if t != 0: # Initial value already defined!
        ...
        # 7 Emissions
        CumulativeEmissions[t]        = CumulativeEmissions[t-1]+(Emissions[t-1])*lengthTimeStep
    ...
    EmisPrice[t]                      = Ramp(t,EmisPriceIncr,50,100)
    OilEmisCost[t]                    = CCOil*EmisPrice[t]
    PriceOfOil[t]                     = (0.1*DepletionCostMultiplier[t]) + OilEmisCost[t]
    ...
    # 7 Emissions
    VolumeOil[t]                    = MarketShareOil[t]*EnergyDemand[t]
    ...
    # 7 Emissions
    Emissions[t]                    = CCOil*VolumeOil[t]

#%% Generate output
data = [CapitalStock,InvestmentFlow,DepreciationFlow,DesiredProductOutput,RefProductCost,EnergyDemand,CumulativeEmissions]
data = np.transpose(data)
label = ['Capital Stock','InvestFlow','DepreciationFlow','DesiredProductOutput','RefProductCost','EnergyDemand','CumulativeEmissions']
